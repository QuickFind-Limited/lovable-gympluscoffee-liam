...
// Test the direct XML edge function
async function testEdgeFunction() {
  const baseUrl = 'http://localhost:54321/functions/v1/supplier-catalog';
  
  try {
    console.log('Testing Direct XML Edge Function\n');
    console.log('================================\n');

    // Test 1: Get suppliers with product counts
    console.log('1. Testing supplier list with product counts...');
    const suppliersUrl = `${baseUrl}?offset=0&limit=5&withProducts=true`;
    ...
    if (!suppliersResponse.ok) {
      console.error('Failed to fetch suppliers:', suppliersResponse.status, suppliersResponse.statusText);
    ...
    console.log('\nSuppliers Response:');
    console.log(JSON.stringify(suppliersData, null, 2));
    ...
    if (suppliersData.suppliers && suppliersData.suppliers.length > 0) {
      ...
      console.log(`\n2. Testing products for supplier ${firstSupplier.name} (ID: ${firstSupplier.id})...`);
      
      const productsUrl = `${baseUrl}?supplierId=${firstSupplier.id}&offset=0&limit=10`;
...
// First, let's create the edge function file locally for testing
import { writeFileSync } from 'fs';
import { execSync } from 'child_process';

const edgeFunctionCode = `import { serve } from 'https://deno.land/std@0.190.0/http/server.ts';
...
// Write the edge function to a file
writeFileSync('/workspaces/source-lovable-animalfarmacy/supabase/functions/supplier-catalog-test/index.ts', edgeFunctionCode);

console.log('Created test edge function at supabase/functions/supplier-catalog-test/index.ts');
console.log('\nTo test this function:');
console.log('1. Start the local Supabase instance: supabase start');
console.log('2. In another terminal, run: supabase functions serve supplier-catalog-test --no-verify-jwt');
console.log('3. Then run this script again to test the endpoints');
console.log('\nOr deploy it directly to test in production.');

// If the function is already running, test it
