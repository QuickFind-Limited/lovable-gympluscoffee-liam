// Test script to check if the original product IDs still work
...
async function testOriginalProductImages() {
  try {
    console.log('=== Testing original product IDs that worked before ===');
    
    // Test with a few known working IDs
    const productIds = [45, 76, 73]; // Acralube, Gloves, Animec
    ...
    if (data.images) {
      ...
      console.log('Images API is working fine with original IDs');
    } else {
      console.log('No images data in response');
    }
    
  } catch (error) {
    console.error('Error fetching original product images:', error);
  }
}

testOriginalProductImages();